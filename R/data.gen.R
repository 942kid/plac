...
#'   one indicator function Zv = I(t >= \eta) is supported); the default is
#'   FALSE.
#' @param Zv.depA logical, whether the time-dependent covariate \code{Zv}
#'   depends on A^* (the only form supported is Zv = I(t >= \eta + A^*)); the
#'   default is FALSE.
...
#' @param scale.T the scale parameter for the Exponential/Weibull distributiof
#'   T*.
...
#'   (\code{Xs}), the observed truncation times (\code{As}), the event indicator
#'   (\code{Ds}) and the covariates (\code{Zs}); a vector of certain quantiles
#'   of Xs (\code{taus}); the censoring proportion (\code{PC}) and the
#'   truncation proportiona (\code{PT}).
#' @examples
#' sim.data()
#' sim.data(n = 50, time.dep = TRUE, distr.A = "binomial", p.A = 0.8, Cmax = 5)
#' @export
sim.data = function(n=200, b = c(1,1), time.dep = FALSE, Zv.depA = FALSE,
                    distr.T = "weibull",
                    shape.T = 2, scale.T = 1,
                    distr.A = "weibull",
	...
	while( j <= n ){
		...
		if( time.dep ){
		  ...
		  eta.j = rexp(1) + ifelse(Zv.depA, As.j, 0)
		  ...
		  if( distr.T == "exp" ){
		    ...
		    t0 = eta.j ^ shape.T
		    Ts.j = (min(eps.j, t0 * RR0.j) / RR0.j / scale.T +
		            max(eps.j - t0 * RR0.j, 0) / RR1.j / scale.T)^(1 / shape.T)
		  }
		}else{
		  Zf.j = runif(p, -1, 1)
		  eta.j = NULL
		  if( distr.T == "exp" ){
		    Ts.j = eps.j * exp(sum(-b * Zf.j)) / scale.T
		  }else if( distr.T == "weibull" ){
		    Ts.j = (eps.j * exp(sum(-b * Zf.j)) / scale.T)^(1 / shape.T)
		  }
		}
...
#' @param X.name the name of the survival time variable in \code{dat}.
...
#' @export
plr = function(dat, A.name = "As", X.name = "Xs", D.name = "Ds"){
  # N-A est. for A and V
  n = nrow(dat)
  names(dat) = gsub(A.name, "As", names(dat))
  names(dat) = gsub(X.name, "Xs", names(dat))
  names(dat) = gsub(D.name, "Ds", names(dat))
  dat$Vs = dat$Xs - dat$As
